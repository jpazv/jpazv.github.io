import { useState, useRef } from "react";
import { Menu, X } from "lucide-react";

export type Section = "hello" | "about" | "projects" | "contact";

const IconX = () => (
  <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
    <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.742l7.736-8.836L1.254 2.25H8.08l4.257 5.622L18.244 2.25zm-1.161 17.52h1.833L7.084 4.126H5.117L17.083 19.77z" />
  </svg>
);
const IconLinkedIn = () => (
  <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
    <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
  </svg>
);
const IconGithub = () => (
  <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
    <path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12" />
  </svg>
);

const SOCIAL = [
  { Icon: IconLinkedIn, url: "https://www.linkedin.com/in/jpazv" },
  { Icon: IconGithub, url: "https://github.com/jpazv" },
];
const NAV_ITEMS: { label: string; section: Section }[] = [
  { label: "_hello", section: "hello" },
  { label: "_about-me", section: "about" },
  { label: "_projects", section: "projects" },
  { label: "_contact-me", section: "contact" },
];

interface MobileLayoutProps {
  activeSection: Section;
  sectionOrder: Section[];
  onNavigate: (s: Section) => void;
  renderSection: (s: Section) => React.ReactNode;
}

export function MobileLayout({
  activeSection,
  sectionOrder,
  onNavigate,
  renderSection,
}: MobileLayoutProps) {
  const [showMenu, setShowMenu] = useState(false);
  const [menuAnim, setMenuAnim] = useState("");
  const [animKey, setAnimKey] = useState(0);
  const [animClass, setAnimClass] = useState("");
  const closingRef = useRef(false);
  const prevSection = useRef<Section>(activeSection);

  const openMenu = () => {
    setShowMenu(true);
    requestAnimationFrame(() => setMenuAnim("mob-menu-in"));
  };

  const closeMenu = () => {
    if (closingRef.current) return;
    closingRef.current = true;
    setMenuAnim("mob-menu-out");
    setTimeout(() => {
      setShowMenu(false);
      setMenuAnim("");
      closingRef.current = false;
    }, 180);
  };

  const handleNav = (section: Section) => {
    if (section === activeSection) { closeMenu(); return; }

    // Calcula direção
    const fromIdx = sectionOrder.indexOf(activeSection);
    const toIdx = sectionOrder.indexOf(section);
    const cls = toIdx > fromIdx ? "mob-anim-right" : "mob-anim-left";

    // 1. Navega imediatamente
    onNavigate(section);
    prevSection.current = section;

    // 2. Fecha menu imediatamente (sem esperar)
    setShowMenu(false);
    setMenuAnim("");
    closingRef.current = false;

    // 3. Dispara animação de entrada do conteúdo
    setAnimClass(cls);
    setAnimKey(k => k + 1);
    setTimeout(() => setAnimClass(""), 280);
  };

  return (
    <>
      <style>{`
        @keyframes mobMenuIn {
          from { opacity: 0; transform: translateY(-8px); }
          to   { opacity: 1; transform: translateY(0); }
        }
        @keyframes mobMenuOut {
          from { opacity: 1; transform: translateY(0); }
          to   { opacity: 0; transform: translateY(-6px); }
        }
        @keyframes mobSlideRight {
          from { opacity: 0; transform: translateX(20px); }
          to   { opacity: 1; transform: translateX(0); }
        }
        @keyframes mobSlideLeft {
          from { opacity: 0; transform: translateX(-20px); }
          to   { opacity: 1; transform: translateX(0); }
        }
        .mob-menu-in    { animation: mobMenuIn    220ms cubic-bezier(0.22,1,0.36,1) both; }
        .mob-menu-out   { animation: mobMenuOut   180ms ease both; }
        .mob-anim-right { animation: mobSlideRight 260ms cubic-bezier(0.22,1,0.36,1) both; }
        .mob-anim-left  { animation: mobSlideLeft  260ms cubic-bezier(0.22,1,0.36,1) both; }
      `}</style>

      <div className="flex flex-col h-full w-full bg-background relative overflow-hidden">

        {/* Header */}
        <header className="flex items-center justify-between px-4 h-11 border-b border-border shrink-0 bg-background z-30 relative">
          <span className="font-mono text-sm text-foreground">joao-moreira</span>
          <button
            onClick={() => showMenu ? closeMenu() : openMenu()}
            className="text-muted-foreground hover:text-foreground transition-colors p-1"
          >
            <span className="block" style={{
              transition: "transform 200ms ease",
              transform: showMenu ? "rotate(90deg)" : "rotate(0deg)",
            }}>
              {showMenu ? <X className="w-5 h-5" /> : <Menu className="w-5 h-5" />}
            </span>
          </button>
        </header>

        {/* Conteúdo — sempre montado */}
        <div className="flex-1 flex flex-col min-h-0">
          <div className="px-4 py-2 border-b border-border shrink-0">
            <span className="font-mono text-xs text-muted-foreground">
              {NAV_ITEMS.find(n => n.section === activeSection)?.label ?? `_${activeSection}`}
            </span>
          </div>

          <main className={`flex-1 min-h-0 ${activeSection === "hello" ? "overflow-hidden" : "overflow-y-auto"}`}>
            <div key={animKey} className={`h-full ${animClass}`}>
              {renderSection(activeSection)}
            </div>
          </main>

 <footer className="px-4 py-4 border-t border-border flex items-center gap-3 shrink-0">
  <span className="font-mono text-xs text-muted-foreground">find me in:</span>
  <div className="flex items-center gap-3">
    {SOCIAL.map(({ Icon, url }, i) => (
      <a key={i} href={url} target="_blank" rel="noopener noreferrer"
        className="text-muted-foreground hover:text-foreground transition-colors p-1">
        <Icon />
      </a>
    ))}
  </div>
</footer>
        </div>

        {/* Menu overlay — por cima */}
        {showMenu && (
          <div className={`absolute inset-0 top-11 z-20 flex flex-col bg-background ${menuAnim}`}>
            <div className="flex-1 px-4 py-6 overflow-y-auto">
              <div className="font-mono text-sm text-muted-foreground mb-5"># navigate:</div>
              <nav className="space-y-px">
                {NAV_ITEMS.map(({ label, section }, i) => (
                  <button
                    key={section}
                    onClick={() => handleNav(section)}
                    className={`w-full text-left font-mono text-base px-4 py-3 transition-colors ${activeSection === section
                        ? "text-foreground bg-secondary/30"
                        : "text-muted-foreground hover:text-foreground hover:bg-secondary/10"
                      }`}
                    style={{
                      animation: menuAnim === "mob-menu-in"
                        ? `mobMenuIn 260ms cubic-bezier(0.22,1,0.36,1) ${i * 40}ms both`
                        : undefined,
                    }}
                  >
                    {label}
                  </button>
                ))}
              </nav>
            </div>
            <footer className="px-4 py-4 border-t border-border flex items-center gap-3 shrink-0">
              <span className="font-mono text-xs text-muted-foreground">find me in:</span>
              <div className="flex items-center gap-3">
                {SOCIAL.map(({ Icon, url }, i) => (
                  <a key={i} href={url} target="_blank" rel="noopener noreferrer"
                    className="text-muted-foreground hover:text-foreground transition-colors p-1">
                    <Icon />
                  </a>
                ))}
              </div>
            </footer>
          </div>
        )}
      </div>
    </>
  );
}